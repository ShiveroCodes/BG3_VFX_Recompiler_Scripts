...
def change_color(input_file_name, target_color, hue_shift):
    new_file_name = input_file_name.replace("Green", target_color)
...
def process_directory(directory_path):
    hue_shifts = {
        "Blue": 240,  # Verschiebung von Grün zu Blau -60
        "Red": 20,  # Verschiebung von Grün zu Rot
        "Purple": -60,  # Verschiebung von Grün zu Lila
        "Yellow": 60,  # Verschiebung von Grün zu Gelb
    }
    for file_name in os.listdir(directory_path):
        if file_name.endswith(".lsfx.lsx") and "Green" in file_name:
