...
def change_color(input_file_name, target_color, hue_shift):
    new_file_name = input_file_name.replace("Yellow", target_color)
...
def process_directory(directory_path):
    hue_shifts = {
        "Blue": 180,  # Verschiebung von Gelb zu Blau
        "Red": 320,  # Verschiebung von Gelb zu Rot
        "Green": 60,  # Verschiebung von Gelb zu Grün
        "Purple": 270,  # Verschiebung von Gelb zu Lila
    }
    for file_name in os.listdir(directory_path):
        if file_name.endswith(".lsfx.lsx") and "Yellow" in file_name:
